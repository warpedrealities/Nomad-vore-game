...
public class SystemScreen extends Screen implements SystemCallback {
	...
	Player player;
	WidgetSystem system;
	java.util.List<SystemDisplay> systemDisplays;
	Window window;
	List itemList;
	Text weightValue;
	Callback callback;
	...
	public SystemScreen(WidgetSystem widgetSystem) {
		...
		systemDisplays=new ArrayList<SystemDisplay>();
		int index=0;
		for (int i=0;i<this.system.getShipAbilities().size();i++)
		{
	...
	public void update(float DT) {
		...
		{
			systemDisplays.get(i).update(DT);
		}
	}
	...
	public void draw(FloatBuffer buffer, int matrixloc) {
		...
		for (int i=0;i<systemDisplays.size();i++)
		{
			systemDisplays.get(i).getWindow().Draw(buffer, matrixloc);
		}
	...
	public void discard(MouseHook mouse) {
		for (int i=0;i<systemDisplays.size();i++)
		{
			systemDisplays.get(i).discard(mouse);
		}
...
//	}
	...
	public void start(MouseHook hook) {
		...
		for (int i=0;i<systemDisplays.size();i++)
		{
			hook.Register(systemDisplays.get(i).getWindow());
		}
...
//		}
	...
	public void initialize(int[] textures, Callback callback) {
		...
		// 3 is button alt
		// 4 tint
		window = new Window(new Vec2f(-20, -16), new Vec2f(23, 2), textures[1], true);
