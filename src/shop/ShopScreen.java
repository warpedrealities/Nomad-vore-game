...
public class ShopScreen extends Screen implements Callback
{
	...
	//	deduct currency
		if (shopData.isUseCredits())
		{
			player.getInventory().setPlayerCredits((int) (player.getInventory().getPlayerCredits()-(item.getCost()*DEF_CREDIT_TO_GOLD)));
		}
		else
		{
	...
	public void Callback() {
		...
		description.addText(Universe.getInstance().getLibrary().readItemDesc(shopList.get(itemLists[1].getSelect()).getName()));
	}
	...
	void resetList()
	{
		...
		{
			str[i]=player.getInventory().getItem(i).getName()+" " +
					""+calcCost(player.getInventory().getItem(i));
