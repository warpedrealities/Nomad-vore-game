...
public class SelectionList extends GUIBase {
	...
	/*
	 * void pushString(String string,int texid) { for (int
	 * i=m_maxcount-1;i>0;i--) { if (m_fonts[i-1]!=null) {
	 * m_fonts[i]=m_fonts[i-1]; //reposition all fonts m_fonts[i].AdjustPos(new
	 * Vec2f(m_pos.x,m_pos.y+((float)i*0.5F))); } } m_fonts[0]=new NuFont(m_pos,
	 * texid, 64, 0.5F); m_fonts[0].setString(string); }
	 */
	...
	public void Draw(FloatBuffer buffer, int matrixloc) {
		...
		for (int i = 0; i < m_maxcount; i++) {
			if (m_fonts[i] != null) {
				...
				// m_font.drawString(m_pos.x, m_pos.y+(20*i),
				// m_strings[i],Color.white);
				if (i == m_selection) {
					// set tint to default
