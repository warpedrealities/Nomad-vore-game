...
public class Widget_Trap extends Widget {
	...
	public boolean Visit()
	{
		if (checkDone==false)
		{
			int r=Universe.m_random.nextInt(20)+Universe.getInstance().getPlayer().getRPG().getAttribute(Actor_RPG.PERCEPTION);
			if (r>DC)
			{
	...
	public boolean Interact(Player player)
	{
		if (discovered)
		{
			int r=Universe.m_random.nextInt(20)+Universe.getInstance().getPlayer().getRPG().getAttribute(Actor_RPG.SECURITY);
			if (r>DC)
			{
