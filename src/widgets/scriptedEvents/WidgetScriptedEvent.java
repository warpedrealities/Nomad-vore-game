...
public class WidgetScriptedEvent extends Widget {
	...
	public boolean Step()
	{
		if (!scriptTriggered)
		{
			...
			try
			{
				LuaValue script = globals.load(new FileReader("assets/data/scripts/" + scriptName + ".lua"), "main.lua");
				script.call();
				LuaValue view = CoerceJavaToLua.coerce(ViewScene.m_interface);
				LuaValue scriptedTools = CoerceJavaToLua.coerce(new ScriptedTools(Universe.getInstance().getCurrentZone()));
