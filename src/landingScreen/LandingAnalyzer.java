...
public class LandingAnalyzer {
		...
		for (int i=0;i<world.getNumZones();i++)
		{
			...
			if (zone.isSurfaceZone())
			{
				...
				if (zone.getTiles()!=null)
				{
					...
					{
						elements.add(new LandingElement(LandingClass.LC_OCCUPIED,(int)zone.getPosition().x,(int)zone.getPosition().y));
					}
					else
					{
						elements.add(new LandingElement(LandingClass.LC_OPEN,(int)zone.getPosition().x,(int)zone.getPosition().y));	
					}
				}
				else
				{
					elements.add(new LandingElement(LandingClass.LC_INVALID,(int)zone.getPosition().x,(int)zone.getPosition().y));
				}
			}
		}
	...
	static private boolean isOccupied(World world, int x, int y)
	{
		if (world.getLandings().size()>0)
		{
			...
			{
				if (world.getLandings().get(i).getX()==x &&
					world.getLandings().get(i).getY()==y)
				{
					return true;
				}
