...
public class CombatShield {
	...
	public enum ShieldStatus{OFF,ON,STARTUP,COOLDOWN,FASTCHARGE};
	...
	public void update()
	{
		if (isActive())
		{
			...
			if (hasEnergy())
			{
				ship.getShipStats().getResource("ENERGY").setResourceAmount(ship.getShipStats().getResource("ENERGY").getResourceAmount()-shield.getEnergyCost());
			}
			else
			{
				shutdown();
