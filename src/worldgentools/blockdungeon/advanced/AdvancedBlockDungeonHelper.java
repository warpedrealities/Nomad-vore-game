...
public class AdvancedBlockDungeonHelper {

	static int getBlock(ArrayList<AdvancedBlock> blocks, int mask, boolean exclusiveMask, short [] edgeTypes) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			if ((blocks.get(i).getEdgeValue() & mask) == mask) {
				if (blocks.get(i).matchEdges(edgeTypes))
				{
		...
		for (int i = 0; i < zone.getWidth(); i++) {
			if (grid[i][0] != null) {
				if (grid[i][0].getDefinition().getBlockVision() == false) {
					grid[i][0] = new Tile(i, 0, zone.getZoneTileLibrary().getDef(wall), zone,
							zone.getZoneTileLibrary());
				}
			}
			if (grid[i][zone.getHeight() - 1] != null) {
				if (grid[i][zone.getHeight() - 1].getDefinition().getBlockVision() == false) {
					grid[i][zone.getHeight() - 1] = new Tile(i, zone.getHeight() - 1,
							zone.getZoneTileLibrary().getDef(wall), zone, zone.getZoneTileLibrary());
				}
			}
		}
		...
		for (int i = 0; i < zone.getHeight(); i++) {
			if (grid[0][i] != null) {
				if (grid[0][i].getDefinition().getBlockVision() == false) {
					grid[0][i] = new Tile(0, i, zone.getZoneTileLibrary().getDef(wall), zone,
							zone.getZoneTileLibrary());
				}
			}
			if (grid[zone.getWidth() - 1][i] != null) {
				if (grid[zone.getWidth() - 1][i].getDefinition().getBlockVision() == false) {
					grid[zone.getWidth() - 1][i] = new Tile(zone.getWidth() - 1, i,
							zone.getZoneTileLibrary().getDef(wall), zone, zone.getZoneTileLibrary());
				}
			}
		}
			...
			for (int j = 1; j < zone.getHeight() - 1; j++) {
				if (grid[i][j] == null) {
					if (adjacentTransparent(grid, i, j)) {
						grid[i][j] = new Tile(i, j, zone.getZoneTileLibrary().getDef(wall), zone,
								zone.getZoneTileLibrary());
