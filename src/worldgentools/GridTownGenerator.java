...
public class GridTownGenerator {
	...
	public GridTownGenerator (Tile [][] tiles,boolean [][]grid,TileDefLibrary library, Zone zone, PointsOfInterest pointsOfInterest)
	{
		m_library=library;
		m_tiles=tiles;
	...
	private Vec2i getPrefabLocation(int width, int height)
	{
		while (true)
		{
			int rx=GameManager.m_random.nextInt(houseGrid.length);
			int ry=GameManager.m_random.nextInt(houseGrid[0].length);
			if (rx<houseGrid.length-width && 
				ry<houseGrid[0].length-height)
			{
	...
	private void buildPrefab(Element element)
	{
		...
		for (int i=0;i<children.getLength();i++)
		{
			...
			if (node.getNodeType()==Node.ELEMENT_NODE)
			{
				...
				{
					doors.add(new Vec2i(x+Integer.parseInt(Enode.getAttribute("x")),y+Integer.parseInt(Enode.getAttribute("y"))));
				}
		...
		for (int i=0;i<doors.size()-1;i++)
		{
			...
			if (pathNodes!=null)
			{
				for (int j=0;j<pathNodes.length;j++)
				{
					...
					{
						m_tiles[(int)node.m_position.x][(int)node.m_position.y]=new Tile((int)node.m_position.x,(int)node.m_position.y,
								m_library.getDef(path),m_zone,m_library);		
					}
