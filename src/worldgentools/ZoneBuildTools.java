...
public class ZoneBuildTools {
			...
			if (Nnode.getNodeType()==Node.ELEMENT_NODE)
			{
				...
				if (Enode.getTagName()=="row")
				{
					...
					for (int j=0;j<width;j++)
					{
						...
						if (value>0)
						{
							m_tiles[x+j][yindex]=new Tile(x+j, yindex, m_library.getDef(value-1), m_zone, m_library);					
						}
	...
	WidgetPortal Portal(Element portalnode, int offsetx,int offsety)
	{
		...
		{
			destination=new Vec2f(Integer.parseInt(portalnode.getAttribute("destinationx")),Integer.parseInt(portalnode.getAttribute("destinationy")));
		}	
		WidgetPortal portal=new WidgetPortal(Integer.parseInt(portalnode.getAttribute("sprite")),string,destination,portalnode.getTextContent().replace("\n", ""));
		...
		for (int i=0;i<children.getLength();i++)
		{
			...
			if (node.getNodeType()==Node.ELEMENT_NODE)
			{
				...
				if (Enode.getTagName()=="auditpaths")
				{
					...
					tool.runPathCarver(Integer.parseInt(Enode.getAttribute("carve")), true,true,true,replace, widgetselection);					
				}
				if (Enode.getTagName()=="blockDungeon")
				{
					new BlockDungeonGenerator(m_zone,m_tiles).run(Enode);
				...
				if (Enode.getTagName().equals("POICornerDecorate"))
				{
					...
					new DecoratorTool(m_tiles,m_library,m_zone).
					runCorners(pointsOfInterest.getCurrentPOI(),child,Integer.parseInt(Enode.getAttribute("floor")));
				}
	...
	void Degrade(Element node, boolean[][]grid)
	{
		...
		tool.Run(Integer.parseInt(node.getAttribute("chance")), 
				Integer.parseInt(node.getAttribute("degradethis")),
