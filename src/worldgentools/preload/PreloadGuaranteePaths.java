...
public class PreloadGuaranteePaths {
	...
	private void runNorth(int tile, int replace, ArrayList<Integer> list) {
		...
		for (int i = 0; i < 2; i++) {
			Vec2i nearest = ClosestIslandTileFinder.getClosestNorth(island, m_zone.getWidth(), m_zone.getHeight(), p[0],
					16);
			if (nearest != null) {
	...
	private void runEast(int tile, int replace, ArrayList<Integer> list) {
		...
		CarvingPathfinder carver = new CarvingPathfinder(m_zone, replace);
		for (int i = 0; i < 2; i++) {
			Vec2i nearest = ClosestIslandTileFinder.getClosestEast(island, m_zone.getWidth(), m_zone.getHeight(), p[0],
					16);
			if (nearest != null) {
				// use carving pathfinder
				carver.carve(nearest, p[i], tile);
	...
	private void runSouth(int tile, int replace, ArrayList<Integer> list) {
		...
		CarvingPathfinder carver = new CarvingPathfinder(m_zone, replace);
		for (int i = 0; i < 2; i++) {
			Vec2i nearest = ClosestIslandTileFinder.getClosestSouth(island, m_zone.getWidth(), m_zone.getHeight(), p[0],
					16);
			if (nearest != null) {
				// use carving pathfinder
				carver.carve(nearest, p[i], tile);
	...
	private void runWest(int tile, int replace, ArrayList<Integer> list) {
		...
		CarvingPathfinder carver = new CarvingPathfinder(m_zone, replace);
		for (int i = 0; i < 2; i++) {
			Vec2i nearest = ClosestIslandTileFinder.getClosestWest(island, m_zone.getWidth(), m_zone.getHeight(), p[0],
					16);
			if (nearest != null) {
				// use carving pathfinder
				carver.carve(nearest, p[i], tile);
