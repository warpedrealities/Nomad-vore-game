...
public class PreloadUtilizer {
	...
	private void carveNorthEdge(ZonePreloadArray preload, int tile,int replace)
	{
		...
		for (int i=0;i<m_zone.getWidth();i++)
		{
			...
			{
				m_zone.getTiles()[i][y]=new Tile(i,y,m_zone.getZoneTileLibrary().getDef(tile),m_zone,m_zone.getZoneTileLibrary());
				m_zone.getTiles()[i][y-1]=new Tile(i,y-1,m_zone.getZoneTileLibrary().getDef(tile),m_zone,m_zone.getZoneTileLibrary());
				count++;
				if (count>=3)
				{
			...
			{
				m_zone.getTiles()[i][y]=new Tile(i,y,m_zone.getZoneTileLibrary().getDef(0),m_zone,m_zone.getZoneTileLibrary());	
				count=0;
			}
		}
	}
	private void carveEastEdge(ZonePreloadArray preload, int tile,int replace)
	{
		...
		for (int i=0;i<m_zone.getWidth();i++)
		{
			if (preload.getPreloadArray().get(i)>0)
			{
				m_zone.getTiles()[x][i]=new Tile(x,i,m_zone.getZoneTileLibrary().getDef(tile),m_zone,m_zone.getZoneTileLibrary());
				m_zone.getTiles()[x-1][i]=new Tile(x-1,i,m_zone.getZoneTileLibrary().getDef(tile),m_zone,m_zone.getZoneTileLibrary());	
				count++;
				if (count>=3)
				{
			...
			{
				m_zone.getTiles()[x][i]=new Tile(x,i,m_zone.getZoneTileLibrary().getDef(0),m_zone,m_zone.getZoneTileLibrary());	
				count=0;
			}
		}		
	}
