...
public class CarvingPathfinder {
	...
	public void carve(Vec2i source, Vec2i destination,int replace)
	{
		...
		path=new Pathfinder_carving(zone,carvethrough).genPath(
				new Vec2f(source.x,source.y),
		...
		if (path!=null)
		{
			for (int i=0;i<path.length;i++)
			{
				...
				if (t!=null)
				{
					...
					{
						zone.getTiles()[(int)path[i].m_position.x][(int)path[i].m_position.y]=
								new Tile((int)path[i].m_position.x,(int)path[i].m_position.y,
