...
public class AuditTool {
	...
	private void createPoints(boolean widgets, boolean npcs, boolean pointsOfInterest,int exclude)
	{
		...
		if (npcs==true)
		{
			...
			{
				points.add(new Vec2i((int)zone.getActors().get(i).getPosition().x,(int)zone.getActors().get(i).getPosition().y));
			}
		}
		if (pointsOfInterest==true)
		{
			...
			{
				points.add(new Vec2i(this.pointsOfInterest.pointList.get(i).x,this.pointsOfInterest.pointList.get(i).y));
			}
		}
	}
	...
	public void runPathCarver(int tunneledTile, boolean widgets, boolean npcs, boolean pointsOfInterest,int replace, int exclude)
	{
