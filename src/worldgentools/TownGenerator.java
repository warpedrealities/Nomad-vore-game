...
public class TownGenerator {
	...
	public TownGenerator (Tile [][] tiles,boolean [][]grid,TileDefLibrary library, Zone zone, PointsOfInterest pointsOfInterest)
	{
		m_library=library;
		m_tiles=tiles;
		...
		for (int i=0;i<houses.size();i++)
		{
			...
			while (true)
			{
						...
						m_tiles[(int) p.x][(int) p.y].getDefinition().getMovement()==TileDef.TileMovement.BLOCK)
				{
					...
					if (m_tiles[(int) p0.x][(int) p0.y]!=null && 
							m_tiles[(int) p0.x][(int) p0.y].getDefinition().getMovement()==TileDef.TileMovement.BLOCK)
					{
						
					}
					else
					{
						m_tiles[(int)p.x][(int)p.y]=new Tile((int)p.x, (int)p.y, m_library.getDef(gap), m_zone, m_library);
						m_tiles[(int)p0.x][(int)p0.y]=new Tile((int)p0.x, (int)p0.y, m_library.getDef(gap), m_zone, m_library);
						break;
					}
				}
				p.x+=v.x;
	...
	private void BuildHouses(int housecount, int minsize, int maxsize, int tile)
	{
		while (houses.size()<housecount)
		{
			...
			//pick a spot
			Rectangle r=new Rectangle(GameManager.m_random.nextInt(m_width),GameManager.m_random.nextInt(m_height),0,0);
			r.setSize(GameManager.m_random.nextInt(maxsize-minsize)+minsize, GameManager.m_random.nextInt(maxsize-minsize)+minsize);
			if (ValidSpot(r))
			{
				m_failcount=0;
				houses.add(r);
	...
	private void PaintHouse(Rectangle r, int tile)
	{
		...
		for (int i=r.getY();i<=r.getY()+r.getHeight();i++)
		{
			...
			m_tiles[r.getX()+r.getWidth()][i]=new Tile(r.getX()+r.getWidth(), i, m_library.getDef(tile), m_zone, m_library);	
		}
