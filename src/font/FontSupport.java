...
FloatBuffer verticesbuffer=BufferUtils.createFloatBuffer(4 * 6* length);	
		...
		for (int i=0;i<text.length();i++)
		{
			...
			verts[0]=new Vertex(x, 0, 0,
					glyph.getCorner().x, glyph.getCorner().y);
			verts[1]=new Vertex(x+glyph.getSize().x*8, 0,
					0, glyph.getCorner().x+glyph.getSize().x,  glyph.getCorner().y);	
			...
			verts[3]=new Vertex(x, glyph.getSize().y*-8 , 0,
					glyph.getCorner().x,  glyph.getCorner().y+glyph.getSize().y);
