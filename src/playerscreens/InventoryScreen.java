...
public class InventoryScreen extends Screen implements Callback {
	...
	public InventoryScreen(int frame, int button, int buttonalt, Player player, int tint,
		...
		for (int i = 0; i < str.length; i++) {
			str[i] = m_player.getInventory().getItem(i).getName() + " "
					+ m_player.getInventory().getItem(i).getWeight();
		}
		m_list.GenList(str);
		...
		m_encumbrance = new Text(new Vec2f(3, -14.9F), "weight:" + Integer.toString(m_player.getInventory().getWeight())
				+ "/" + Integer.toString(m_player.getInventory().getCapacity()), 0.7F, tint);
		encumbranceWarning = new TextColoured(new Vec2f(3, -15.6F), "warning", 0.7F, tint);
		calculateWarning();
		currencyTexts = new Text[2];
		currencyTexts[0] = new Text(new Vec2f(8, -14.9F), "credits:" + m_player.getInventory().getPlayerCredits(), 0.7F,
				tint);
		currencyTexts[1] = new Text(new Vec2f(8, -15.6F), "gold:" + m_player.getInventory().getPlayerGold(), 0.7F,
				tint);
		...
		if (m_control > 0) {
			...
			if (ItemBlueprintInstance.class.isInstance(item)) {
				...
				ViewScene.m_interface.DrawText(
						"you examine the plans on the blueprints and have unlocked the recipe for " + ibi.getRecipe());
				m_player.setBusy(2);
				m_dropdown.setVisible(false);
			}
	...
	void ResetList() {
		if (screenAlive) {
			...
			m_encumbrance.setString("weight:" + Integer.toString(m_player.getInventory().getWeight()) + "/"
					+ Integer.toString(m_player.getInventory().getCapacity()));
			m_list.GenList(str);
		...
		for (int i = 0; i < m_player.getInventory().getNumItems(); i++) {
			if (ItemAmmo.class.isInstance(m_player.getInventory().getItem(i).getItem())) {
				if (item.getEnergy().getRefill().contains(m_player.getInventory().getItem(i).getItem().getName())) {
					...
					if (ammo.getEnergy() > 0) {
						m_player.TakeAction();
						// drain item to try and replenish that which we're
						// recharging
	...
	void Info() {
		...
		if (m_control > 0) {
			if (ItemExpositionInstance.class.isInstance(m_player.getInventory().getItem(m_control - 1))) {
				...
				try {
					popupBig.setString(
							ParserHelper.readString("assets/data/exposition/" + iei.getExposition() + ".txt"));
				} catch (IOException e) {
	...
	void HandleDropdown() {
		...
		case 1:
			...
			if (m_dropdownstrings[1].equals("split")
					&& ItemStack.class.isInstance(m_player.getInventory().getItem(m_control - 1))) {
