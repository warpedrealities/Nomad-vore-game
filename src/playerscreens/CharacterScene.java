...
//		m_textureIds[2]=Tools.loadPNGTexture("assets/art/spritesheet.png", GL13.GL_TEXTURE0);
	//	m_textureIds[3]=Tools.loadPNGTexture("assets/art/"+m_world.currentZone.getTileset(),  GL13.GL_TEXTURE0);
	...
	private void genButtons( int tint, int button,int buttonalt)
	{
		...
		//buttons[3]=new Button(Vec2f pos, Vec2f size,  int texture, int font, MyListener listener,String string, int ID, float s);
	...
	private void genExperienceNotes(int font, int tint)
	{
		...
		experienceNotes.setString("level "+playerRPG.getPlayerLevel()+" exp "+playerRPG.getPlayerExperience()+"/"+playerRPG.getNextLevel());
	}
	...
	private void genAttributePage(int frame, int font,int tint)
	{
		...
		text[4]=new Text(new Vec2f(5.5F,1.0F),"intelligence:"+playerRPG.getAbility(Actor_RPG.INTELLIGENCE),0.8F,tint);
