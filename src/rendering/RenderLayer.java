	...
	public void Create(Tile_Int [][] tiles, int count, int offsetx, int offsety)
	{
		...
		for (int i=0;i<16;i++)
		{
			for (int j=0;j<16;j++)
			{
				...
				{
					if (tiles[i+offsetx][j+offsety].getImage()>0 && tiles[i+offsetx][j+offsety].getExplored()==true)
					{
		...
		GL20.glVertexAttribPointer(1, 2, GL11.GL_FLOAT, false, Vertex_ext.size,
		4 * 4);
		GL20.glVertexAttribPointer(2, 1, GL11.GL_FLOAT, false, Vertex_ext.size,
		4 * 6);	
	...
	public void CreateWidgets(Tile_Int [][] tiles, int count, int offsetx, int offsety)
	{
		...
		for (int i=0;i<16;i++)
		{
			for (int j=0;j<16;j++)
			{
				if (tiles[i+offsetx][j+offsety]!=null)
				{
					if (tiles[i+offsetx][j+offsety].getWidget()>0 && tiles[i+offsetx][j+offsety].getExplored()==true)
					{
						//generate square,
