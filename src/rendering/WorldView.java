	...
	public void vision(Zone zone,ArrayList <Actor> m_actors, Vec2f p)
	{
		...
		for (int i=0;i<m_width;i++)
		{
			...
			for (int j=0;j<m_height;j++)
			{
				...
				{
	//				m_layers[i][j].generateOverlay(zone.getTiles(),16*i, 16*j,overlayController);
				}
				...
				if (zone.getTiles()[i][j]!=null)
				{
					if (zone.getTiles()[i][j].getWidgetObject()!=null)
					{
						...
						if (str.contains("Sprite"))
						{
							...
							SegSprite sprite=new SegSprite(widget.getImage(),tiles,new Vec2f(i,j-(h*0.5F)-1.0F),width,height);
