	...
	public void update(ArrayList<Actor> m_actors, Vec2f pos) {
		...
		for (int i = minx; i < maxx; i++) {
			for (int j = miny; j < maxy; j++) {
				if (zone.getTile(i, j) != null && zone.getTile(i, j).getExplored()
						&& zone.getTile(i, j).getOverlayImage() > -1 && zone.getTile(i, j).getWidgetObject() == null) {
					grid[i][j] = true;
				} else {
					grid[i][j] = false;
				}
		...
		for (int i = 0; i < m_actors.size(); i++) {
			if (m_actors.get(i).getVisible() && m_actors.get(i).getPosition().x > minx
					&& m_actors.get(i).getPosition().x < maxx && m_actors.get(i).getPosition().y > miny
