...
//	ZoneInteractionHandler m_handler;
	...
	public ViewScene(int[] variables, Universe world) {
		...
		m_buttons[4]=new Button2(new Vec2f(0.5F,0.5F),new Vec2f(8,2),m_textureIds[7],this, "look",4,m_textureIds[8],1);
		m_window.add(m_buttons[4]);	
		
		
	//	m_handler=new ZoneInteractionHandler(sceneController.getActiveZone(),this);
		
		//m_screen=new InventoryScreen(m_textureIds[1],m_textureIds[0],m_textureIds[7],m_textureIds[8],m_world.m_player,m_variables[0],this);
	...
	void SetupTextures()
	{
		...
		m_textureIds[3]=Tools.loadPNGTexture("assets/art/"+sceneController.getActiveZone().getTileset(),  GL13.GL_TEXTURE0);
	...
	void addTime()
	{
		if (Keyboard.isKeyDown(GLFW.GLFW_KEY_LEFT_SHIFT  )||Keyboard.isKeyDown(GLFW.GLFW_KEY_RIGHT_SHIFT)
				||Keyboard.isKeyDown(GLFW.GLFW_KEY_LEFT_CONTROL)||Keyboard.isKeyDown(GLFW.GLFW_KEY_RIGHT_CONTROL)||screenFade.active())
		{
			m_time=0.025F;
		}
		else
			...
			{	
				if (sceneController.getUniverse().player.CanAct())
				{
					if (targeter.isActive())
					{
						...
						{
							addTime();
							m_view.vision(sceneController.getActiveZone(),sceneController.getActiveZone().zoneActors,sceneController.getUniverse().getPlayer().getPosition());
							redrawBars();
					...
					else
					{
						if (sceneController.getUniverse().player.Control()||Click())
						{
							...
							{
								sceneController.getActiveZone().zoneTileGrid
								[(int)sceneController.getUniverse().player.getPosition().x]
										[(int)sceneController.getUniverse().player.getPosition().y].Step();
								m_viewmatrix.m30=0;m_viewmatrix.m31=0;
								
								m_viewmatrix.translate(new Vector2f(-1*sceneController.getUniverse().player.getPosition().x-9,
										-1*sceneController.getUniverse().player.getPosition().y+7));		
								sceneController.getActiveZone().ClearVisibleTiles();
								GameManager.m_vision.visitFieldOfView(sceneController.getActiveZone(), (int)sceneController.getUniverse().player.getPosition().x, 
										(int)sceneController.getUniverse().player.getPosition().y, 9);
...
//						sceneController.getHandler().Seduce(p,sceneController.getUniverse().player);
	...
	public Vec2f Transition(String destination, int id)
	{
		...
		m_viewmatrix.translate(new Vector2f(-1*sceneController.getUniverse().player.getPosition().x-9,-1*sceneController.getUniverse().player.getPosition().y+7));	
	...
	void Transition(int side,String destination)
	{
		...
		case 0:
			...
			sceneController.getUniverse().player.setPosition(
					new Vec2f(sceneController.getUniverse().player.getPosition().x,1));
			break;
		case 1:
			//player to the east edge
			sceneController.getUniverse().player.setPosition(
					new Vec2f(1,sceneController.getUniverse().player.getPosition().y));
			
			break;
		case 2:
			//player to north edge
			sceneController.getUniverse().player.setPosition(
					new Vec2f(sceneController.getUniverse().player.getPosition().x,sceneController.getActiveZone().zoneHeight-2));
			break;
		case 3:
			//player to west edge
			sceneController.getUniverse().player.setPosition(
					new Vec2f(sceneController.getActiveZone().zoneWidth-2,sceneController.getUniverse().player.getPosition().y));
			break;
		}
		Safety(side);
		sceneController.getActiveZone().RegenZone();
		...
		//run vision algorithm		
		sceneController.getActiveZone().ClearVisibleTiles();
		GameManager.m_vision.visitFieldOfView(
				sceneController.getActiveZone(), 
				(int)sceneController.getUniverse().player.getPosition().x, 
				(int)sceneController.getUniverse().player.getPosition().y, 9);
		m_view.End();
		//reset the view
...
//		m_view.Draw(m_variables[2], matrix44Buffer);
	...
	public void ButtonCallback(int ID, Vec2f p) {
		...
		if (m_screen==null && m_dropdown.getVisible()==false)
		{
			...
			{
				case 0:
				m_screen=new InventoryScreen(m_textureIds[0],m_textureIds[7],m_textureIds[8],sceneController.getUniverse().player,m_variables[0],this);
				m_screen.start(m_hook);
				break;
				case 1:	
					//appearance
	...
	boolean PlaceItemPile(int x, int y, Item item)
	{
		if (sceneController.getActiveZone().zoneTileGrid[x][y].getWidgetObject()!=null)
		{
			...
			if (sceneController.getActiveZone().zoneTileGrid[x][y].getWidgetObject().getClass().getName().contains("ItemPile"))
			{
				WidgetItemPile pile=(WidgetItemPile)sceneController.getActiveZone().zoneTileGrid[x][y].getWidgetObject();
				pile.AddItem(item);
				return true;
			}
		}
