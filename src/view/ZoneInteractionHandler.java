...
public class ZoneInteractionHandler {
	...
	public boolean Look(Vec2f p)
	{
		...
		if (m_zone.zoneTileGrid[(int) p.x][(int) p.y]!=null && m_zone.zoneTileGrid[(int) p.x][(int) p.y].getActorInTile()!=null)
		{
		...
		for (int i=0;i<m_zone.zoneActors.size();i++)
		{
			...
			int x1=(int)m_zone.zoneActors.get(i).getPosition().x;
			int y1=(int)m_zone.zoneActors.get(i).getPosition().y;
			if (x0==x1 && y0==y1 && m_zone.zoneActors.get(i).getVisible()==true)
			{
				//post 
			...
			if (CanTalk(player))
			{
				...
				if (InteractActor(p,player)==false)
				{
					...
					if (m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()!=null)
					{
						...
						player.TakeAction();
						if (WidgetBreakable.class.isInstance(m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()))
						{
							WidgetBreakable w=(WidgetBreakable)m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject();
...
/*
	boolean MeleeAttack(Vec2f p, Player player)
	{
		int xt=(int)p.x;
		int yt=(int)p.y;
		//check widget
		if (m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()!=null)
		{
			if (Attackable.class.isInstance(m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()))
			{
				Attackable attackable=(Attackable)m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject();
				//attack this		
				attackable.Harm(player.getAttack().getDamage(0), player,0);
				m_view.Flash(new Vec2f(xt,yt),0);
				player.TakeAction();
				violationCheck(attackable.getName(),p,ViolationType.Attack);
				return true;
			}	
		}
		//check for actor in tile
		for (int i=0;i<m_zone.zoneActors.size();i++)
		{
			...
			{
				int x=(int)m_zone.zoneActors.get(i).getPosition().x;
				int y=(int)m_zone.zoneActors.get(i).getPosition().y;
				if (xt==x && yt==y)
				{
					//conduct attack
	...
	boolean RangedAttack(Vec2f p, Player player)
	{
		//check widget
		int xt=(int)p.x;
		int yt=(int)p.y;
		//check line of sight
		if (GameManager.m_los.existsLineOfSight((ILosBoard)m_zone,(int)player.getPosition().x, (int)player.getPosition().y,(int)p.x, (int)p.y, true))
		{
			if (m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()!=null)
			{
				if (Attackable.class.isInstance(m_zone.zoneTileGrid[(int)p.x][(int)p.y].getWidgetObject()))
	...
	{
		attack.Attack attack=player.getAttack();
		if (attack.getRanged()==true)
		{
			//handle ranged
			RangedAttack(p, player);
		}
		else if (player.getPosition().getDistance(p)<2)
		{
			//handle melee
	...
	public void Seduce(Vec2f p, Player player)
	{
		int xt=(int)p.x;
		int yt=(int)p.y;
		for (int i=0;i<m_zone.zoneActors.size();i++)
		{
			if (m_zone.zoneActors.get(i)!=player && m_zone.zoneActors.get(i).getVisible()==true && m_zone.zoneActors.get(i).getAttackable())
			{
				...
				if (xt==x && yt==y)
				{
					...
					{				
						//roll damage
						int dmg=GameManager.m_random.nextInt(6)+player.getRPG().getAttribute(Actor_RPG.SEDUCTION);
						//reduce by distance
						dmg=dmg-(int)(player.getPosition().getDistance(m_zone.zoneActors.get(i).getPosition()));
						//apply to actor
						if (dmg<0)
						{
							dmg=0;
						}
						int strength=m_zone.zoneActors.get(i).Weaken(dmg, Actor_RPG.TEASE, player);
						//text message go
						String str="You seduce "+m_zone.zoneActors.get(i).getName()+" and they lose "+Integer.toString(strength)+" resolve";
	...
	*/
	...
	{
		if (GameManager.m_los.existsLineOfSight((ILosBoard)m_zone,(int)player.getPosition().x, (int)player.getPosition().y,x, y, true))
		{
		...
		//			return true;
			...
			//check for actor in tile
			if (m_zone.zoneTileGrid[x][y]!=null 
					&& m_zone.zoneTileGrid[x][y].getActorInTile()!=null 
