...
public class TextView extends GUIBase {
	...
	public void AddText(String text)
	{
		lastInput=text;
		highlightedLine=0;
		float divisions=(float)text.length()/(125/Game.sceneManager.getConfig().getTextscale());
		//divide text
		...
		if (divisions>1)
		{
			...
			{
				//find split point
				int split=text.indexOf(" ", index+(int)(120/Game.sceneManager.getConfig().getTextscale()));
				if (split==-1)
				{
					break;
				}
				if (split>index+125/Game.sceneManager.getConfig().getTextscale())
				{
					split=text.indexOf(" ", index+(int)(115/Game.sceneManager.getConfig().getTextscale()));
				}
				String str=text.substring(index, split);
				int dex=str.indexOf("LBREAK");
				if (dex!=-1)
				{
					str=str.substring(0,dex);
					str=str.replace("LBREAK", "");
					split=index+dex+"LBREAK".length();
					i--;
				}
			...
			String str=text.substring(index,text.length());
			int dex=str.indexOf("LBREAK");
			while (dex!=-1)
			{
				str=str.replace("LBREAK", "");
				displayStrings.add(str.substring(0,dex));
				index=dex+index+1;
				str=text.substring(index,text.length());
				dex=str.indexOf("LBREAK");
				highlightedLine++;
			}
