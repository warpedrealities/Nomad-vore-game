...
function launch(script,miss)
	print("launch")
		...
		target=script:leadTarget(distance/64)
		if ( miss == true ) then
			target.x=target.x+math.random(0,2)-1
			target.y=target.y+math.random(0,2)-1
		end
		script:makeSprite(0,emitter:replicate(),0,2,true,1)
		script:makeSprite(1,emitter:replicate(),4,4,false,1)	
		script:getEffect(0):setAnimationSpeed(2)
