...
function launch(script,miss)
		...
		target=script:leadTarget(distance/32)
		if ( miss == true ) then
	...
	angle=script:getAngle(emitter.x-target.x,target.y-emitter.y)
...
function fly(script,miss,scriptClock)
	...
	if (distance<1) then
		script:resolve()
		script:makeSprite(2,script:getEffect(0):getPosition():replicate(),8,4,false,4)		
